...
def process_files():
    ...
    for i in range(S, T+1):
        ...
        script_path = os.path.join(api_call_dir, 'node_type.py')
        ...
        try:
            ...
            subprocess.run(['python', 'node_type.py'], check=True)
            print("Successfully executed node_type.py")
            
        except subprocess.CalledProcessError as e:
            print(f"Error executing node_type.py: {e}")
            # 恢复原始目录
