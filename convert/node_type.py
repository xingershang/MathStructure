# step 2. node_type each sentence
...
file_node_type_current = "../display/node_type_current.md"

# clear files
with open(file_node_type_context_whole, "w", encoding="utf-8") as f:
    f.write("")
    print(f"Cleared {file_node_type_context_whole}")
with open(file_node_type_result_whole, "w", encoding="utf-8") as f:
    f.write("")
    print(f"Cleared {file_node_type_result_whole}")
...
while iteration < max_iterations:
    ...
    with open(file_node_type_context_whole, "r", encoding="utf-8") as f:
        node_type_context_whole = f.read().strip()
    with open(file_node_type_result_whole, "r", encoding="utf-8") as f:
        node_type_result_whole = f.read().strip()
    
    messages = [{"role": "system", "content": node_type_prompt}]
    messages.append({"role": "user", "content": f"自然语言文本（已切分）: \n{input_content}\n"})
    messages.append({"role": "user", "content": f"历史Round: \n{node_type_context_whole}\n"})
    messages.append({"role": "user", "content": f"本轮之前的识别汇总: \n{node_type_result_whole}\n"})
    ...
    try:
        ...
        with open(file_node_type_llm_response, "w", encoding="utf-8") as f:
        ...
        if thinking or result:
            ...
            with open(file_node_type_current, "w", encoding="utf-8") as f:
                f.write(result)
